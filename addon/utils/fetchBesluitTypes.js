...
const RDF_TYPE = new rdflib.NamedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#type');
const CONCEPT_CLASS = new rdflib.NamedNode('http://www.w3.org/2004/02/skos/core#Concept');
const DECIDABLE_BY = new rdflib.NamedNode('http://lblod.data.gift/vocabularies/besluit/decidableBy');
const SKOS_PREFLABEL = new rdflib.NamedNode('http://www.w3.org/2004/02/skos/core#prefLabel');
const SKOS_BROADER = new rdflib.NamedNode('http://www.w3.org/2004/02/skos/core#broader');
const SKOS_TOP_CONCEPT_OF = new rdflib.NamedNode('http://www.w3.org/2004/02/skos/core#topConceptOf');
const TYPE_CONCEPT_SCHEME = new rdflib.NamedNode('https://data.vlaanderen.be/id/conceptscheme/BesluitType');
...
function findConceptsDecidableBy(graph, classificationUri) {
  ...
  return dataset
    .filter((triple) => graph.holds(triple.subject, DECIDABLE_BY, classification))
...
export default async function fetchBesluitTypes(classificatieUri) {
  const graph = await readUrlIntoGraph('/assets/ttl/20210205102900-new-besluit-types.ttl');
